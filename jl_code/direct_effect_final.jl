# ===================================================================
# ANALYSIS SCRIPT (Final, Direct Effect for All Sectors)
...
# ===================================================================
# PART 3: ROLLING WINDOW DIRECT EFFECT ANALYSIS
# ===================================================================

println("\n--- Part 3: Setting up the Rolling Window Analysis ---")

# Define all 7 wage sectors as dependent variables (LHS)
all_wage_sectors = ["wage_construction", "wage_manufacturing", "wage_retail", "wage_adminwaste", "wage_healthsocial", "wage_highskill", "wage_entertainment"]
...
println("\n--- Starting Rolling Regression Loop ---")
...
for end_year in 2016:2022
    ...
    key_vars = vcat(all_wage_sectors, "stations_per_100k_capita", "stations_per_100k_capita_lag1")
    ...
    timings =      [(name="Unlagged",      station_var="stations_per_100k_capita"), 
                    (name="Lagged",        station_var="stations_per_100k_capita_lag1")]

    # Innermost loops for sectors and models
    for sector in all_wage_sectors
        for timing in timings
            ...
            vars_needed = [lhs_var, station_var_full, :population, :migration_individuals, :county_fips, :year]
            temp_df = dropmissing(model_df, vars_needed)

            if nrow(temp_df) > 50
                model = reg(temp_df, @eval(@formula($(lhs_var) ~ $(station_var_full) + population + migration_individuals + fe(county_fips) + fe(year))))
                
                ct = coeftable(model)
                coef_row = findfirst(x -> x == string(station_var_full), ct.rownms)
...
println("EVOLUTION OF DIRECT EFFECT COEFFICIENTS (Sample):")
println(first(results_over_time, 14))

results_output_path = joinpath(tables_path, "rolling_direct_effect_results.csv")
...
results_to_plot.ci_lower = results_to_plot.station_coefficient .- 1.96 .* results_to_plot.std_error
results_to_plot.ci_upper = results_to_plot.station_coefficient .+ 1.96 .* results_to_plot.std_error

title_map = Dict("wage_construction"=>"Construction", "wage_manufacturing"=>"Manufacturing", "wage_retail"=>"Retail", "wage_adminwaste"=>"Waste Mgmt", "wage_healthsocial"=>"Healthcare", "wage_entertainment"=>"Entertainment", "wage_highskill"=>"High-Skill")
...
for (spec_name, spec_group) in pairs(groupby(results_to_plot, :model_spec))
    ...
    for sector in all_wage_sectors
        ...
        if !isempty(sector_df)
            p = plot(sector_df.end_year, sector_df.station_coefficient, label="", title=get(title_map, sector, sector), xlabel="", ylabel="", ribbon=(sector_df.station_coefficient .- sector_df.ci_lower, sector_df.ci_upper .- sector_df.station_coefficient), fillalpha=0.2)
    ...
    if !isempty(all_sector_plots)
        final_plot = plot(all_sector_plots..., layout=(4, 2), size=(1000, 1400), plot_title="Direct Effect Evolution: $(spec_name.model_spec) Model")
        output_plot_path = joinpath(plots_path, "rolling_direct_effects_$(spec_name.model_spec).png")
