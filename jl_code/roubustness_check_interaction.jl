# ===================================================================
# ANALYSIS SCRIPT (Final, Per-Capita Only)
# --- Rolling Window with 6-Model Specification Analysis ---
# ===================================================================
...
function standardize(v)
    s = std(skipmissing(v))
    if s == 0 || ismissing(s) || isnan(s)
        return zeros(length(v))
    else
        return (v .- mean(skipmissing(v))) ./ s
    end
end
...
for end_year in 2016:2022
    ...
    key_vars = vcat(lhs_sectors, rhs_sector, "stations_per_100k_capita")
    ...
    vars_to_transform = vcat(key_vars, "stations_per_100k_capita_lag1", "population", "migration_individuals")
    for col_str in vars_to_transform
        col_sym = Symbol(col_str)
        if col_str in names(model_df)
            log_col = Symbol("log_", col_str); model_df[!, log_col] = log.(model_df[!, col_sym] .+ 1)
            std_col = Symbol("std_", col_str); model_df[!, std_col] = standardize(model_df[!, col_sym])
        end
    end

    # Define the specifications for the inner loops
    specs =        [(name="Level",         prefix=""), 
                    (name="Log",           prefix="log_"), 
                    (name="Standardized",  prefix="std_")]
    timings =      [(name="Unlagged",      suffix=""), 
                    (name="Lagged",        suffix="_lag1")]

    # Innermost loops for sectors and models
    for sector in lhs_sectors
        for spec in specs
            for timing in timings
                model_name = "$(spec.name) - $(timing.name)"
                
                station_var_name = "stations_per_100k_capita" * timing.suffix
                
                lhs_var = Symbol(spec.prefix * sector)
                rhs_var = Symbol(spec.prefix * rhs_sector)
                station_var_full = Symbol(spec.prefix * station_var_name)
                pop_var = Symbol(spec.prefix * "population")
                mig_var = Symbol(spec.prefix * "migration_individuals")
                
                vars_needed = [lhs_var, rhs_var, station_var_full, pop_var, mig_var, :county_fips, :year]
                temp_df = dropmissing(model_df, vars_needed)

                if nrow(temp_df) > 50
                    model = reg(temp_df, @eval(@formula($(lhs_var) ~ $(rhs_var) * $(station_var_full) + $(pop_var) + $(mig_var) + fe(county_fips) + fe(year))))
...
results_output_path = joinpath(tables_path, "rolling_interaction_results_percapita.csv")
...
# Create a separate plot for each of the 6 model specifications
for (spec_name, spec_group) in pairs(groupby(results_to_plot, :model_spec))
    ...
    if !isempty(all_sector_plots)
        final_plot = plot(all_sector_plots..., layout=(3, 2), size=(1000, 1200), plot_title="Interaction Effect Evolution: $(spec_name.model_spec)")
        output_plot_path = joinpath(plots_path, "rolling_effects_percapita_$(spec_name.model_spec).png")
